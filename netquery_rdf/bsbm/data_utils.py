...
import random
from netquery_rdf.data_utils import parallel_sample, load_queries_by_type, sample_clean_test

from netquery_rdf.graph import Graph, Query, _reverse_edge

def load_graph(data_dir, embed_dim):
    rels, adj_lists, node_maps = pickle.load(open(data_dir+"/bsbm100_new.pkl", "rb"))
    adj_lists = dict(adj_lists)
    fine_rels = defaultdict(list)
    for t in rels:
        fine_rels[t[0]].append((t[1], t[2]))
        if t[2] not in fine_rels:
            fine_rels[t[2]] = []
    # node_maps = {m : {n : i for i, n in enumerate(id_list)} for m, id_list in node_maps.items()}
    node_maps = {m: {id_list[n]: i for i, n in enumerate(id_list)} for m, id_list in node_maps.items()}
    for m in node_maps:
        node_maps[m][-1] = -1
    feature_dims = {m : embed_dim for m in fine_rels}
    feature_modules = {m : torch.nn.Embedding(len(node_maps[m])+1, embed_dim) for m in fine_rels}
    for mode in fine_rels:
        feature_modules[mode].weight.data.normal_(0, 1./embed_dim)
    features = lambda nodes, mode : feature_modules[mode](
            torch.LongTensor([node_maps[mode][n] for n in nodes])+1)
    graph = Graph(features, feature_dims, fine_rels, adj_lists)
